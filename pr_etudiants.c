...
void score(DIGRAPH *g, int k){
    double *s=malloc(g->n*sizeof(double)); //score actuel s
    double *s_suivant=malloc(g->n*sizeof(double)); //score de l'iteration precedente 

    for (int i=0; i<g->n; i++) s[i]=1.0/g->n; //initialisation de s0
    printf("s0: ");
    for (int i=0; i<g->n; i++) printf("%f ", s[i]);
    printf("\n");
